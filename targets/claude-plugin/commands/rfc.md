...
## close
Set the final status (Accepted / Rejected / Deferred), record the decision and key dissent, then **hand off**: open an ADR via `adr-writer` (`/adr-new`) to record the decision immutably, and flag the SDD section (`sdd-writer`) that must change. The ADR — not the RFC — is the durable record.
