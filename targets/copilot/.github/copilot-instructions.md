...
- **Layout standard:** the canonical `docs/` + `.spindleloom/` tree, the profiles, and the four cadence planes (durable/living/cyclic/snapshot) are fixed by `knowledge_hub/GOVERNANCE.md` Part I; existing repos convert via `scaffold.py migrate`, never by hand.
...
The full standard (feedback loops, change control, team-size tiers, frameworks) is in `knowledge_hub/BEST-PRACTICES.md` (bundled). Navigate the fleet by phase in `AGENTS.md` / `agents/INDEX.md`.
