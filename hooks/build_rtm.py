...
> appends missing IDs and never touches existing rows.

| Req-ID | Kind | Defined in | Downstream | Test | Status |
|---|---|---|---|---|---|
"""

ROW_RE = re.compile(r"(?m)^\|\s*`?([A-Z][A-Z0-9]{1,7}-[A-Z0-9]{2,12}-\d{1,4})`?\s*\|")
...
def main(argv):
    ...
    existing = set()
    if rtm.exists():
        existing = set(ROW_RE.findall(rtm.read_text(encoding="utf-8", errors="ignore")))
