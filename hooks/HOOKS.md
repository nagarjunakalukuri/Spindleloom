...
| `mcp_server.py` | **Spindleloom MCP server** (FastMCP) — exposes the live traceability graph **and artifact catalog** as MCP tools (`trace_requirement`, `rtm_coverage`, `list_requirements`, `find_decision`, `list_artifacts`, `find_artifact`) + resources (`rtm://current`, `spindleloom://requirements`, `spindleloom://artifacts`) so any MCP-aware harness (Claude Code, Cursor, Copilot, Windsurf) queries specs live instead of reading static markdown. Reads `$SPINDLELOOM_SPEC_ROOT`. The **one** component with a dependency (`pip install "mcp[cli]"`), isolated here — `rtm_core`, the validator, and the registry generator stay stdlib-only. | Launched by a harness via the generated `.mcp.json`. |
