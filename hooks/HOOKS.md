...
| `validate_graph.py` | **Self-check for the fleet** — the toolkit eating its own docs-as-code. Verifies **(1)** contract-graph *symmetry* (every `downstream` edge reciprocated in the target's `upstream`, and vice versa), **(2)** no *dangling* agent refs (an upstream/downstream name with no agent file), **(3)** no *missing skills* (a `skills:` entry with no `skills/<name>/SKILL.md`), **(4)** `INDEX.md` *freshness* (one row per agent), and **(5)** every agent carries `examples:` prompts (the `/help-role` source). Prints live agent/template/skill counts as advisories. Read-only, stdlib-only, exit 1 on problems. | After adding/editing/removing/renaming an agent, and as a CI/pre-PR gate. |
| `rtm_core.py` | **Shared traceability + artifact core** (stdlib-only, read-only). Resolves the docs location (`resolve_docs_root` — `.shipwright/config.json` `docs_root`, default `docs/`, fallback flat), then parses it (recursively, excluding dotdirs) into the requirement graph (Req-IDs, RTM matrix + decisions, ADR refs) *and* the artifact catalog (kind from path incl. parent folder, owner/status/version/last-updated from doc headers). Exposes `audit()`, `trace()`, `list_requirements()`, `find_decision()`, `parse_rtm()`, `artifacts()`, `find_artifact()`, `resolve_docs_root()`. Imported by the validator, the registry generator, and the MCP server — one parser, no drift. | Library; not run directly. |
| `build_artifact_registry.py` | **Artifact catalog generator** (stdlib-only) — the *retrieval* layer the RTM doesn't cover. Takes a **project root**, resolves `docs_root`, discovers every artifact, and writes the catalog into **`.shipwright/`** (machinery, out of the human docs tree): `.shipwright/artifacts.json` + `ARTIFACTS.md` (id, kind, title, path, owner, status, version, last-updated, Req-IDs defined). `--init` bootstraps `.shipwright/config.json`; `--baseline <tag>` freezes a version/status snapshot to `.shipwright/baselines/<tag>.json`; `--check` exits 1 if stale. Answers "what artifacts exist, where, owned by whom, in what state?" | Against a project root; after spec changes; `--check` in CI. |
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python project_managment_agents/hooks/validate_reqs.py \"$CLAUDE_PROJECT_DIR\"/$(dirname \"${CLAUDE_TOOL_INPUT_file_path#$CLAUDE_PROJECT_DIR/}\") 2>/dev/null || true"
          }
        ]
      }
    ]
        ...
        "hooks": [
          {
            "type": "command",
            "shell": "bash",
            "command": "out=$(py \"$CLAUDE_PROJECT_DIR/hooks/validate_graph.py\" 2>&1); ec=$?; if [ $ec -ne 0 ]; then echo \"$out\" >&2; exit 2; fi; if ! py \"$CLAUDE_PROJECT_DIR/hooks/build_harness_artifacts.py\" --check >/dev/null 2>&1; then printf '{\"systemMessage\":\"Wheelwright harness bundles are stale vs source — run: py hooks/build_harness_artifacts.py to resync targets/.\"}'; fi",
            "timeout": 30,
...
- name: Validate spec traceability
  run: |
    for d in $(git diff --name-only origin/main... | grep -E '/RTM\.md$' | xargs -n1 dirname | sort -u); do
      python project_managment_agents/hooks/validate_reqs.py "$d"
    done
```
...
- name: Validate agent-fleet integrity
  run: |
    python project_managment_agents/hooks/build_agent_index.py   # keep INDEX current
    python project_managment_agents/hooks/validate_graph.py       # symmetry, dangling refs, skills, INDEX, claude_code mappings, commands
    python project_managment_agents/hooks/build_harness_artifacts.py --check   # per-tool bundles not stale vs source
```
...
- **Cache location:** `~/.cache/wheelwright/sdd/` by default. Override: `"env": {"SDD_CACHE_DIR": "/your/path"}`.
