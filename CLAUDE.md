...
Spindleloom is **not an application** — it's a fleet of **52 role-specialist AI agents** (plus 52 templates, 28 skills, 23 slash commands, self-validating Python hooks, and a traceability MCP server) that carry a product across the full SDLC (market → spec → design → build → test → ship → operate) as one traceable chain. The "code" here is mostly **Markdown-as-contract** (`agents/*.md`, `templates/*.md`, `skills/*/SKILL.md`, `commands/*.md`) plus **stdlib-only Python** in `hooks/` that validates and generates from it.
...
- **`validate_graph.py`** runs 13 checks — chiefly **contract-graph symmetry** (every agent you list in `downstream` must list you in their `upstream`, and vice versa), no dangling refs, no missing skills, INDEX freshness, `claude_code:` mapping integrity, and harness-data sync.
...
In *this* repo (the toolkit), `sloom check` runs the fleet battery: graph symmetry + handoff freshness + bundle drift. In an *adopter's* repo it runs the spec battery (reqs + RTM + registry + gates). `sloom reqs|rtm|gates|pack|registry|context|run|approve|scaffold|backlog|graph|targets|index` forward to the underlying tools. The `run`/`approve`/`context`/`flags` quartet drives the multi-user concurrency + phase-acceptance layer: `sloom run` is the distributed-run ledger (status/advance), `sloom approve` writes a phase-boundary acceptance token, `sloom context` imports/exports handoff context across a pull, and `sloom flags` surfaces open FLAG(owner) re-work. The toolkit `sloom check` also runs `validate_counts` (no stale fleet numbers in the docs), `validate_personas` (the persona hub matches the contracts), and `build_governance_handbook --check` (the reading view matches GOVERNANCE.md).
...
py -3 hooks/build_governance_handbook.py # refresh the governance reading view
py -3 hooks/build_fleet_page.py          # refresh the fleet map data from the contracts
...
| `knowledge_hub/` | The authoritative conventions (see below) |
...
- **`knowledge_hub/AGENT-AUTHORING.md`** — the contract-block schema and prompting conventions. **Read this before creating or editing any agent.**
- **`knowledge_hub/GOVERNANCE.md`** — the merged governance standard: Part I the authoritative, versioned layout standard for adopter repos (the `docs/` + `.spindleloom/` tree, profiles, cadence planes, ID immutability); Part II story craft (INVEST, AC, splitting); Part III team + Azure DevOps fit. Part I secs 12-15 carry the operational detail (golden rule, baselines, retrieval, tools).
- **`knowledge_hub/BEST-PRACTICES.md`** — the funnel, feedback loops, team-size tiers, the requirement-quality standard (ISO/IEC/IEEE 29148 + INCOSE, the "system shall" rule), and the Req-ID traceability backbone.
- **`knowledge_hub/HARNESS-MATRIX.md`** — the 7-surface × 4-tool matrix that `build_harness_artifacts.py` targets. Consult when changing what generates into `targets/`.
