...
phase: test
inputs: [test plan, API contract]
outputs: automated test suites
rtm_column: "—"
...
> **Handoff** · *Before:* read test plan, API contract (from `test-plan-writer`, `api-designer`, `flaky-test-detective`). *After:* produce automated test suites → hand to `ci-cd-pipeline`. *(Flag discoveries back upstream — see `project_guides/BEST-PRACTICES.md`.)*
...
3. Write the automated tests with stable selectors, controlled data (test-data), and proper waits; wire them into the ci-cd-pipeline at the right stage (PR vs nightly).
...
## Who participates
QA/SDETs and developers build automation; test-plan-writer sets what to verify; ci-cd-pipeline runs it as gates; flaky-test-detective keeps it trustworthy; qa-tester focuses manual effort where automation can't reach.
