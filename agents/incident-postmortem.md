...
phase: operate
inputs: [release-plan, alerts, incident-timeline]
outputs: postmortem
id_prefix: INC
rtm_column: "—"
upstream: [release-manager, sre]
downstream: [backlog-manager, retrospective-facilitator, tech-debt-register, raid-log]
skills: [systematic-debugging, observability-slo-design, production-incident-response]
claude_code: { subagent_type: incident-postmortem }
---

> **Handoff** · *Before:* read release-plan, alerts, incident-timeline (from `release-manager`, `sre`). *After:* produce postmortem → hand to `backlog-manager`, `retrospective-facilitator`, `tech-debt-register`, `raid-log`. *(Flag discoveries back upstream — see `project_guides/BEST-PRACTICES.md`.)*
