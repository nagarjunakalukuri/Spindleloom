...
id_prefix: RFC
rtm_column: "—"
upstream: [sdd-writer, architect]
downstream: [adr-writer, sdd-writer]
skills: [architecture-decision-framing]
claude_code: { command: /rfc, subagent_type: rfc }
---

> **Handoff** · *Before:* read SDD, ADR, SRS (from `sdd-writer`, `architect`). *After:* produce RFC → hand to `adr-writer`, `sdd-writer`. *(Flag discoveries back upstream — see `project_guides/BEST-PRACTICES.md`.)*
