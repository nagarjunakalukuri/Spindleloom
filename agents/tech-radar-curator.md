...
1. Seed from what already exists: scan ADRs (decisions made), `coding-standards-writer` (the sanctioned stack), and the TSD (chosen tech) — place each as a blip in its quadrant/ring with a rationale.
