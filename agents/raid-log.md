...
id_prefix: RAID
rtm_column: "—"
upstream: [solution-recon, architect, sprint-planner, incident-postmortem]
downstream: [status-reporter]
skills: []
claude_code: { subagent_type: raid-log }
---

> **Handoff** · *Before:* read BRD, SRS, ADR (from `solution-recon`, `architect`, `sprint-planner`, `incident-postmortem`). *After:* produce raid-log → hand to `status-reporter`. *(Flag discoveries back upstream — see `project_guides/BEST-PRACTICES.md`.)*
