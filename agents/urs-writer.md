...
> **Handoff** · *Before:* read BRD (from `doc-strategy-advisor`, `brd-writer`). *After:* produce URS → hand to `srs-writer`, `frd-writer`, `prd-writer`. *(Flag discoveries back upstream — see `knowledge_hub/BEST-PRACTICES.md`.)*
...
4. Draft using the template below. Give every requirement a `URS-AREA-NNN` ID and a verification method, and run each through the 29148/INCOSE quality checklist (`knowledge_hub/BEST-PRACTICES.md`).
