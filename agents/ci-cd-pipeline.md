...
phase: release
inputs: [TSD, coding-standards, definition-of-ready-done-template]
outputs: ci-cd-pipeline
rtm_column: "—"
upstream: [test-automation-engineer, coding-standards, code-reviewer, security-reviewer, ai-eval]
downstream: [release-manager, flaky-test-detective, sre]
skills: [agentic-loop-design, eval-harness-design, observability-slo-design]
claude_code: { subagent_type: ci-cd-pipeline }
---

> **Handoff** · *Before:* read TSD, coding-standards, definition-of-ready-done-template (from `test-automation-engineer`, `coding-standards`, `code-reviewer`, `security-reviewer`, `ai-eval`). *After:* produce ci-cd-pipeline → hand to `release-manager`, `flaky-test-detective`, `sre`. *(Flag discoveries back upstream — see `project_guides/BEST-PRACTICES.md`.)*
...
## Who participates
DevOps/leads design and own it; the architect sets deploy topology (from the SDD/TSD); all developers depend on it; it enforces coding-standards and the DoD automatically.

## Feedback loop
A single red build is the author's to fix, but a recurring failure at one gate is a signal worth routing upstream. A lint or format stage that keeps tripping points at a coding-standards gap to flag to coding-standards; a flaky or repeatedly-failing test stage points at a test-coverage or test-quality gap to flag to test-automation-engineer. Sending the pattern back to the owning agent fixes the cause rather than re-running the pipeline.
