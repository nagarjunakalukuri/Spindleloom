...
- **Context layers — one home per kind of context.** Three stores exist while a run executes; the same fact in two of them is a defect (the golden rule applied to context): **facts/decisions of record** → the docs tree + RTM/ADRs (durable, reviewed); **compressed working notes** — decisions-in-flight with reasons, output paths, blockers, inherited constraints, resolved incantations → the `sloom` context memory (`save_context`/`recall_context`, `.spindleloom/context.db`), ≤5 bullets per entry, scoped by `task_id`; **orchestration state** — stop contract, ledger, gates → `.spindleloom/run-state.json`. Ad-hoc handoff files (a shared log everyone appends to) are the anti-pattern: lossy, unscoped, and invisible to `validate_gates --context`.
